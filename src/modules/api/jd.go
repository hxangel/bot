...
func (c *Jd) Item() {
	...
	spiderServ.Add("JdItem", id, callback)
...
func (c *Jd) Shop() {
	...
	spiderServ.Add("JdShop", id, callback)
	c.Json(0, "success", "success")
}
...
func (c *Jd) Samestyle() {
	...
	surl := fmt.Sprintf("http://s.jd.com/search?q=%s", title)
	fmt.Println(surl)
	...
	url := fmt.Sprintf("http://s.jd.com/list?tab=all&type=samestyle&uniqpid=-%s&app=i2i&nid=%s", pid, id)
	fmt.Println(url)
